DROP TABLE IF EXISTS review;
DROP TABLE IF EXISTS shipment;
DROP TABLE IF EXISTS productinventory;
DROP TABLE IF EXISTS warehouse;
DROP TABLE IF EXISTS orderproduct;
DROP TABLE IF EXISTS incart;
DROP TABLE IF EXISTS product;
DROP TABLE IF EXISTS category;
DROP TABLE IF EXISTS ordersummary;
DROP TABLE IF EXISTS paymentmethod;
DROP TABLE IF EXISTS customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(100),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(1000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);


INSERT INTO category(categoryName) VALUES ('Jazz');
INSERT INTO category(categoryName) VALUES ('Electronic');
INSERT INTO category(categoryName) VALUES ('Funk/Soul');
INSERT INTO category(categoryName) VALUES ('Hip Hop');
INSERT INTO category(categoryName) VALUES ('Rock');
INSERT INTO category(categoryName) VALUES ('Soundtrack');

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Young Holt Unlimited - Just A Melody', 1, '(Brunswick Records) [1968] LP Album',47.94);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Floating Points, Pharoah Sanders & The London Symphony Orchestra – Promises',1,'(Luaka Bop Records) [2021] LP Album',26.28);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Makaya McCraven – Deciphering The Message',1,'(Blue Note Records) [2021] LP Album',17.58)
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Herbie Hancock – Head Hunters',1,'[1973] (Columbia Records) LP Album',34.95);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Moses Boyd – Dark Matter',1,'[2020] (Exodus Records) LP Album',17.73);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Alice Coltrane Featuring Pharoah Sanders – Journey In Satchidananda',1,'(Impulse! Records) [1971] LP Album',8.37);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Kaidi Tatham – An Insight To All Minds',2,'(First World Records) [2021] LP Album',31.29);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Theo Parrish – Sound Sculptures Volume 1',2,'(Sound Signature Records) [2007] LP Album',14.34);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Daft Punk – Homework',2,'(Virgin Records) [1997] LP Album',41.85);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Mr Fingers – Ammnesia',2,'(Jack Traxx Records) [1989] LP Album',21.51);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Kraftwerk – Computerwelt',2,'(Kling Klang Records) [1981] LP Album',24.17);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('IG Culture Presents Likwid Continual Space Motion – Earthbound',3,'(Super-Sonic Jazz Records) [2020] LP Album',50.53);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Khruangbin – Con Todo El Mundo',3,'(Night Time Stories Recordings) [2018] LP Album',15.17);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('El Michels Affair – Enter The 37th Chamber',3,'(Fat Beats Records) [2009] LP Album',12.91);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Bill Withers – Just As I Am',3,'(Sussex Records) [1971] LP Album',12.65);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('BadBadNotGood – Talk Memory',3,'(XL Recordings) [2021] LP Album',15.20);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Madlib - Sound Ancestors',4,'(Madlib Invazion Records) [2020] LP Album',13.39);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('J Dilla - The Shining',4,'(BBE Records) [2006] LP Album',30.32);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('The Roots – Dilla Joints',4,'(Self Released Record) [2010] LP Album',329.81);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Kendrick Lamar – To Pimp A Butterfly',4,'(Top Dawg Entertainment) [2015] LP Album',90.90);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('DJ Shadow - Endtroducing.....',4,'(Mo Wax Records) [1996] LP Album',47.16);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Stereolab - Dots and Loops',5,'(Duophonic Ultra High Frequency Disks) [1997] LP Album',113.87);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Tame Impala – The Slow Rush',5,'(Island Records) [2020] LP Album',50.08);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Vangelis – Blade Runner',6,'(EastWest Records) [1994] LP Album',85.17);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Flying Lotus – Yasuke',6,'(Warp Records) [2021] LP Album',13.93);

INSERT INTO warehouse(warehouseName) VALUES ('Main warehouse');
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (1, 1, 10, 47.94);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (2, 1, 10, 26.28);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (3, 1, 10, 17.58);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (4, 1, 10, 34.95);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (5, 1, 10, 17.73);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (6, 1, 10, 8.37);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (7, 1, 10, 31.29);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (8, 1, 10, 14.34);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (9, 1, 10, 41.85);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (10, 1, 10, 21.51);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (11, 1, 10, 24.17);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (12, 1, 10, 50.53);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (13, 1, 10, 15.17);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (14, 1, 10, 12.91);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (15, 1, 10, 12.65);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (16, 1, 10, 15.20);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (17, 1, 10, 13.39);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (18, 1, 10, 30.32);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (19, 1, 10, 329.81);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (20, 1, 10, 90.90);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (21, 1, 10, 47.16);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (22, 1, 10, 113.87);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (23, 1, 10, 50.08);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (24, 1, 10, 85.17);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (25, 1, 10, 13.93);


INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'password');
INSERT INTO customer (firstName, lastName, userid, password) VALUES ('admin','', 'admin', 'admin');


-- Order 1 can be shipped as have enough inventory
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2021-12-09 10:25:55', 104.91)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 1, 1, 47.94)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 2, 17.73)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 1, 21.51);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2021-12-08 18:00:00', 88.65)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 5, 17.73);

-- Order 3 cannot be shipped as do not have enough inventory for item 7
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2021-11-15 3:30:22', 110.61)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 2, 8.37)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 7, 3, 31.29);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2021-10-17 05:45:11', 384.91)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 3, 4, 17.58)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 8, 3, 14.34)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 13, 3, 15.17)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 24, 2, 85.17)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 25, 4, 13.93);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (5, '2021-9-15 10:25:55', 277.40)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 1, 17.73)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 19, 1, 329.81)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 20, 1, 141.44);

-- New SQL DDL for lab 8
UPDATE Product SET productImageURL = 'img/1.jpg' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'img/2.jpg' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'img/3.jpg' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'img/4.jpg' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'img/5.jpg' WHERE ProductId = 5;
UPDATE Product SET productImageURL = 'img/6.jpg' WHERE ProductId = 6;
UPDATE Product SET productImageURL = 'img/7.jpg' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'img/8.jpg' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'img/9.jpg' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'img/10.jpg' WHERE ProductId = 10;
UPDATE Product SET productImageURL = 'img/11.jpg' WHERE ProductId = 11;
UPDATE Product SET productImageURL = 'img/12.jpg' WHERE ProductId = 12;
UPDATE Product SET productImageURL = 'img/13.jpg' WHERE ProductId = 13;
UPDATE Product SET productImageURL = 'img/14.jpg' WHERE ProductId = 14;
UPDATE Product SET productImageURL = 'img/15.jpg' WHERE ProductId = 15;
UPDATE Product SET productImageURL = 'img/16.jpg' WHERE ProductId = 16;
UPDATE Product SET productImageURL = 'img/17.jpg' WHERE ProductId = 17;
UPDATE Product SET productImageURL = 'img/18.jpg' WHERE ProductId = 18;
UPDATE Product SET productImageURL = 'img/19.jpg' WHERE ProductId = 19;
UPDATE Product SET productImageURL = 'img/20.jpg' WHERE ProductId = 20;
UPDATE Product SET productImageURL = 'img/21.jpg' WHERE ProductId = 21;
UPDATE Product SET productImageURL = 'img/22.jpg' WHERE ProductId = 22;
UPDATE Product SET productImageURL = 'img/23.jpg' WHERE ProductId = 23;
UPDATE Product SET productImageURL = 'img/24.jpg' WHERE ProductId = 24;
UPDATE Product SET productImageURL = 'img/25.jpg' WHERE ProductId = 25;

-- Loads image data for product 1
UPDATE Product SET productImage =  WHERE ProductId = 1;